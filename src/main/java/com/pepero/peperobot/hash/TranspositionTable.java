...
public class TranspositionTable {
    // hash table size
    private static final int TT_SIZE = 0x2000000;
    ...
    private static int[]  tt_best_moves = new int[TT_SIZE];    // [추가됨] best move 저장용 배열
    ...
    public static void clearTT(){
        ...
        Arrays.fill(tt_best_moves,0); // [추가됨] 초기화
    }
    ...
    // write hash entry data (best_move 파라미터 추가)
    public static void writeHashEntry(Search search, Chessboard chessboard, int score, int depth, int hash_flag, int best_move){
        ...
        tt_best_moves[index] = best_move; // [추가됨] best move 저장
    }
}